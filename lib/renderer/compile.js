...
const compileNext = (distFolder, options) => {
...
const renderToTemplate = (componentString, componentName, data, stateful) => {  
  ...
  return require(path.resolve(__dirname, template))(componentString, componentName, JSON.stringify(data));
};
...
const compile = (componentData, distFolder, options) => {
  ...
  compiler.run((err, stats) => {
    ...
    const statefulGlobal = data['stateful'] ? data['stateful'] : false;
    if (data['config']) {
    ...
    let output = '';
    for (let i in data) {
      let componentData = data[i]['props'] ? data[i]['props'] : data[i];
...
module.exports = (components, distFolder, options) => {
