...
const isStyledAndReturnProps = function(tag) {
  ...
  // styled.global.body``
  if (tag.type === 'MemberExpression' && tag.object.object && tag.object.object.name === styled) {
    if (tag.object.property && tag.object.property.name === 'global') {      
      return {type: 'MemberExpression', tag: tag.property.name, className: null, isGlobal: true};
    }
  }
  // styled('p')``
...
const addGlobalStyles = function(path, props) {
  globalStyles += `
    ${props.tag} {
      ${extractTextFromTemplateLiteral(path.quasi)}
    }
  `;
}
