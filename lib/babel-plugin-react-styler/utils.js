...
const replaceCssPlaceholdersGlobalStyles = function() {
  let hashes = getHashesFromString(globalStyles);
  let result = globalStyles;
  for (let i=0; i<hashes.length; i++) {
    let hash = hashes[i];
    let value = getValueFromGlobalIdentifier(hash);
    if (value !== null) {
...
const isStyledAndReturnProps = function(tag) {
  ...
  // styled.p``
  if (tag.type === 'MemberExpression' && tag.object.name && tag.object.name === styled) {
    // styled.global``
    if (tag.property.name === 'global') {
      return {type: 'MemberExpression', tag: null, className: null, isGlobal: true};
    }
...
const addGlobalStyles = function(path) {
  globalStyles += extractTextFromTemplateLiteral(path.quasi);
}

const getGlobalStyles = function() {
  return globalStyles;
}
...
module.exports = {
  ...
  replaceCssPlaceholdersGlobalStyles,
  getAllCss,
